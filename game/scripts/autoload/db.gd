...
const ITEMS := {
	...
	"cinder_gland": {"name": "Cinder Gland", "kind": "resource", "color": Color("ff6a1a"), "desc": "A Cinder Mite's fuel sac. Handle it gently."},
	"slag_heart": {"name": "Slag Heart", "kind": "resource", "color": Color("c77a2b"), "desc": "The still-glowing firebox of a Smelter."},
	"storm_vane": {"name": "Storm Vane", "kind": "resource", "color": Color("8f86ff"), "desc": "A Storm Kite's wing coil. It crackles when you hold it."},
	...
	"arc_coil": {"name": "Arc Coil", "kind": "upgrade", "color": Color("8f86ff"), "desc": "Unlocks the Arc weapon loadout: bolts that chain between targets and shock them."},
	"cinder_launcher": {"name": "Cinder Launcher", "kind": "upgrade", "color": Color("ff6a1a"), "desc": "Unlocks the Cinder weapon loadout: lobbed grenades that set everything burning."},
	"cryo_projector": {"name": "Cryo Projector", "kind": "upgrade", "color": Color("7fe3ff"), "desc": "Unlocks the Cryo weapon loadout: a held beam that chills targets until they freeze."},
	"pulse_mk2": {"name": "Pulse Mk II", "kind": "upgrade", "color": Color("ffd23f"), "desc": "Retunes the Pulse loadout: +35% damage."},
	"scatter_mk2": {"name": "Scatter Mk II", "kind": "upgrade", "color": Color("ffd23f"), "desc": "Retunes the Scatter loadout: +35% damage."},
	"rail_mk2": {"name": "Rail Mk II", "kind": "upgrade", "color": Color("ffd23f"), "desc": "Retunes the Rail loadout: +35% damage."},
	"arc_mk2": {"name": "Arc Mk II", "kind": "upgrade", "color": Color("ffd23f"), "desc": "Retunes the Arc loadout: +35% damage."},
	"cinder_mk2": {"name": "Cinder Mk II", "kind": "upgrade", "color": Color("ffd23f"), "desc": "Retunes the Cinder loadout: +35% damage."},
	"cryo_mk2": {"name": "Cryo Mk II", "kind": "upgrade", "color": Color("ffd23f"), "desc": "Retunes the Cryo loadout: +35% damage."},
...
const RECIPES := [
	...
	{"id": "arc_coil", "out": "arc_coil", "qty": 1, "in": {"storm_vane": 3, "circuit": 2, "lumen": 4}, "req": 30, "xp": 160, "cat": "Upgrades"},
	{"id": "cinder_launcher", "out": "cinder_launcher", "qty": 1, "in": {"cinder_gland": 4, "slag_heart": 1, "alloy": 6}, "req": 32, "xp": 170, "cat": "Upgrades"},
	{"id": "cryo_projector", "out": "cryo_projector", "qty": 1, "in": {"rime_core": 3, "cryo_ice": 10, "polymer": 2}, "req": 28, "xp": 150, "cat": "Upgrades"},
	{"id": "pulse_mk2", "out": "pulse_mk2", "qty": 1, "in": {"thorn_barb": 4, "sand_fang": 2, "alloy": 4}, "req": 20, "xp": 120, "cat": "Upgrades"},
	{"id": "scatter_mk2", "out": "scatter_mk2", "qty": 1, "in": {"cinder_gland": 3, "thorn_barb": 3, "alloy": 4}, "req": 24, "xp": 130, "cat": "Upgrades"},
	{"id": "rail_mk2", "out": "rail_mk2", "qty": 1, "in": {"rime_core": 2, "prism_lens": 2, "circuit": 2}, "req": 32, "xp": 160, "cat": "Upgrades"},
...
const WEAPONS := {
	...
	"rail": {"name": "Rail", "desc": "Slow, heavy slug that pierces everything in a line.", "rate": 4.0, "dmg": 3.4, "pellets": 1, "spread": 0.0, "range": 1.8, "cost": 4.5, "pierce": true, "unlock": "rail_mod", "kind": "pierce"},
	"arc": {"name": "Arc", "desc": "A crackling bolt that leaps to up to 3 more targets nearby and shocks them all.", "rate": 1.2, "dmg": 0.95, "pellets": 1, "spread": 0.0, "range": 0.7, "cost": 1.8, "pierce": false, "unlock": "arc_coil", "mode": "chain", "kind": "shock"},
	"cinder": {"name": "Cinder", "desc": "Lobs a grenade that bursts into flame, burning everything in the blast.", "rate": 2.4, "dmg": 2.2, "pellets": 1, "spread": 0.0, "range": 1.0, "cost": 3.2, "pierce": false, "unlock": "cinder_launcher", "mode": "lobbed", "kind": "fire"},
	"cryo": {"name": "Cryo", "desc": "Hold fire for a freezing beam. Chill stacks up until the target freezes solid.", "rate": 0.4, "dmg": 0.3, "pellets": 1, "spread": 0.0, "range": 0.5, "cost": 0.45, "pierce": false, "unlock": "cryo_projector", "mode": "beam", "kind": "frost"},
}
...
const ENEMIES := {
	...
	"mite": {"name": "Cinder Mite", "model": "res://assets/models/enemy_mite.glb",
		"hp": [14, 4], "dmg": [12, 3.0], "speed": 9.0, "range": 1.9, "cd": 0.5, "aggro": 22.0, "xp": [14, 5],
		"scale": 0.6, "style": "swarm", "pack": 3, "mods": {"frost": 1.5}, "resist": {"burn": 1.0},
		"loot": {"cinder_gland": [0, 1]},
		"biome": "ember", "tip": "Comes in packs and bursts into flame when it reaches you. Shooting one pops it too, so thin them out at range. Scatter shreds them."},
	"smelter": {"name": "Smelter", "model": "res://assets/models/enemy_smelter.glb",
		"hp": [130, 32], "dmg": [16, 4.0], "speed": 2.6, "range": 28.0, "cd": 2.6, "aggro": 28.0, "xp": [80, 22],
		"scale": 1.0, "style": "mortar", "heavy": true, "mods": {"frost": 1.5}, "resist": {"burn": 1.0},
		"loot": {"slag_heart": [1, 1], "alloy": [0, 2]},
		"biome": "forge", "tip": "Lobs molten shells where you're heading. Its plating shrugs off most damage until it vents steam after every third shot: that's your window."},
	"kite": {"name": "Storm Kite", "model": "res://assets/models/enemy_kite.glb",
		"hp": [55, 15], "dmg": [15, 3.5], "speed": 6.0, "range": 26.0, "cd": 3.2, "aggro": 30.0, "xp": [50, 15],
		"scale": 1.0, "style": "storm", "wades": true, "mods": {"frost": 1.3, "pierce": 1.3}, "resist": {"shock": 1.0},
		"loot": {"storm_vane": [1, 1], "plasma": [0, 2]},
		"biome": "tempest", "tip": "Circles overhead calling down lightning. Keep moving when a ring appears under you; a shock makes every hit hurt more."},
	"refractor": {"name": "Refractor", "model": "res://assets/models/enemy_refractor.glb",
		"hp": [60, 16], "dmg": [11, 2.8], "speed": 4.0, "range": 22.0, "cd": 2.4, "aggro": 24.0, "xp": [52, 16],
		"scale": 1.0, "style": "mirror", "wades": true, "mods": {"shock": 1.5}, "resist": {"burn": 0.5},
		"loot": {"prism_lens": [1, 1], "lumen": [0, 2]},
		"biome": "prism", "tip": "While its facets glow, your shots bounce back at you. Hold fire until the glow fades. It blinks around, so track the flash."},
	"stalker": {"name": "Void Stalker", "model": "res://assets/models/enemy_stalker.glb",
		"hp": [70, 19], "dmg": [15, 3.8], "speed": 7.5, "range": 11.0, "cd": 2.0, "aggro": 20.0, "xp": [60, 18],
		"scale": 1.0, "style": "stalk", "mods": {"shock": 1.3},
		"loot": {"void_veil": [1, 1], "voidshard": [0, 1]},
		"biome": "abyss", "tip": "Nearly invisible until it's close. Scan (Q) to reveal it and leave it exposed for extra damage, then sidestep the pounce."},
	"hive": {"name": "Spore Hive", "model": "res://assets/models/enemy_hive.glb",
		"hp": [150, 38], "dmg": [10, 2.5], "speed": 0.0, "range": 16.0, "cd": 5.0, "aggro": 24.0, "xp": [90, 25],
